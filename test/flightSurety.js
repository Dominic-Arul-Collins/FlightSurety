...
contract('Flight Surety Tests', async (accounts) => {
  ...
  it('multi-party threshold is reached', async () => {
    ...
    try{
        ...
        console.log('Seems there is a airline registration failure');
    }
  ...
  it('Check if airline is only operational once 10 ether is funded', async () => {
    ...
    try{
        ...
        console.log('Airline Ether Funding - Failed');
    }
    
    let result = await config.flightSuretyData.getAirlineOperatingStatus.call(admin3);

    // ASSERT
    assert.equal(result, true, "Airline Operating Status is false") 
  ...
  it('Voting is made when multi-party threshold is reached', async () => {
    ...
    try{
        ...
        if(registration_status[0] == false && registration_status[1] == false){
            await config.flightSuretyApp.approveAirlineRegistration(admin4, true, {from: config.owner});  
            await config.flightSuretyApp.approveAirlineRegistration(admin4, true, {from: admin3});         
            await config.flightSuretyApp.approveAirlineRegistration(admin4, false, {from: admin2});         
        }
  ...
  it('Passenger can buy flight insurance for atmost 1 ether', async () => {
  ...
  it('Passenger can withdraw ether', async () => {
